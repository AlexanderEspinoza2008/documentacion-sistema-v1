# Documentación de Arquitectura del Sistema

## 1. Navegación

Paso 3: Vinculación desde el README principal
1. Abre tu archivo README.md principal.
2. Ve a la Sección 5 (Enlaces Útiles) y agrega el enlace relativo hacia la nueva documentación de arquitectura.
3. El apartado 5 debe quedar redactado exactamente así:

## 5. Enlaces Útiles
- [Ver Arquitectura del Sistema](docs/arquitectura.md)
- [Repositorio Oficial en GitHub](https://github.com/TU_USUARIO/documentacion-sistema-c1)
4. Haz clic en la Vista Previa (Ctrl + Shift + V) y prueba hacer clic en el enlace "Ver Arquitectura del Sistema". Debe abrirse automáticamente el archivo arquitectura.md.